...
rem for /F %%j in ('"%_JAVACMD%" -version  2^>^&1') do (
rem   if %%~j==java set JAVAINSTALLED=1
rem   if %%~j==openjdk set JAVAINSTALLED=1
rem )
...
rem if "%_JAVACCMD%"=="" set _JAVACCMD=javac
rem for /F %%j in ('"%_JAVACCMD%" -version 2^>^&1') do (
rem   if %%~j==javac set JAVACINSTALLED=1
rem )

rem BAT has no logical or, so we do it OLD SCHOOL! Oppan Redmond Style
set JAVAOK=true
rem if not defined JAVAINSTALLED set JAVAOK=false
rem if not defined JAVACINSTALLED set JAVAOK=false
